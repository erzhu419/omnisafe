...
class SAC_Trainer():
    ...
    def update(self, batch_size,training_steps, reward_scale=10., auto_entropy=True, target_entropy=-2, gamma=0.99, soft_tau=1e-2):
        state, action, reward, _cost, next_state, done = self.replay_buffer.sample(batch_size)
...
cost_episodic = []  # 记录每个 episode 的平均 cost

# 创建用于存储评估结果的列表
eval_episodes = []      # 记录进行评估的 episode 编号
eval_mean_rewards = []  # 记录评估的平均奖励
eval_reward_stds = []   # 记录评估的奖励标准差
eval_mean_costs = []    # 记录评估的平均 cost
eval_cost_stds = []     # 记录评估的 cost 标准差
...
if __name__ == '__main__':
    if args.train:
        ...
        # 评估最终策略
        mean_reward, reward_std, mean_cost, cost_std = evaluate_policy(sac_trainer, env, num_eval_episodes=15, deterministic=True)
        print(f"最终评估结果: 平均奖励 = {mean_reward:.2f}, 标准差 = {reward_std:.2f}, 平均Cost = {mean_cost:.2f}")
