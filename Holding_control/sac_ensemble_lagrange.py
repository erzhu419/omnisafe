...
parser.add_argument('--save_root', type=str, default='results_ensemble_lagrange', help='Base directory for saving models')
...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        return np.stack(states), np.stack(actions), np.array(rewards, dtype=np.float32), \
               np.array(costs, dtype=np.float32), np.stack(next_states), np.array(dones, dtype=np.float32)
...
class SAC_Trainer():
    def __init__(self, env, replay_buffer, hidden_dim, action_range, ensemble_size=10):
        ...
        self.cost_q_net = SoftQNetwork(state_dim, action_dim, hidden_dim, embedding_layer, ensemble_size=ensemble_size).to(device)
        self.target_cost_q_net = deepcopy(self.cost_q_net)
        ...
        self.cost_q_criterion = nn.MSELoss()
        ...
        self.cost_q_optimizer = optim.Adam(self.cost_q_net.parameters(), lr=soft_q_lr)
    ...
    def compute_cost_q_loss(self, state, action, cost, next_state, done, new_next_action, next_log_prob, gamma):
        predicted_cost_q_value = self.cost_q_net(state, action)
        with torch.no_grad():
            target_cost_q_next = self.target_cost_q_net(next_state, new_next_action)
            target_cost_q_value = cost + (1 - done) * gamma * target_cost_q_next.unsqueeze(-1)
        
        cost_q_value_loss = self.cost_q_criterion(predicted_cost_q_value, target_cost_q_value.squeeze(-1))
        return cost_q_value_loss, predicted_cost_q_value
    ...
    def compute_policy_loss(self, state, action, new_action, log_prob, reg_norm):
        ...
        cost_q_values_dist = self.cost_q_net(state, new_action)

        q_mean = q_values_dist.mean(dim=0)
        q_std = q_values_dist.std(dim=0)
        q_loss = -(q_mean + args.beta * q_std).mean()
        
        cost_q_max, _ = cost_q_values_dist.max(dim=0)
        lambda_val = F.softplus(self.lambda_param).detach()
        cost_loss = (lambda_val * cost_q_max).mean()

        bc_loss = F.mse_loss(new_action, action)
        # smooth_loss = self.get_policy_smooth_loss(state)

        loss = args.beta_bc * bc_loss + q_loss + cost_loss
    ...
    def update(self, batch_size, training_steps, reward_scale=10., auto_entropy=True, target_entropy=-2, gamma=0.99, soft_tau=1e-2):
        ...
        cost_q_value_loss, predicted_cost_q_value = self.compute_cost_q_loss(state, action, cost, next_state, done, new_next_action, next_log_prob, gamma)
        ...
        cost_q_value_loss.backward(retain_graph=False)
        if args.use_gradient_clip:
            torch.nn.utils.clip_grad_norm_(self.cost_q_net.parameters(), max_norm=1.0)
        ...
        for target_param, param in zip(self.target_cost_q_net.parameters(), self.cost_q_net.parameters()):
            target_param.data.copy_(target_param.data * (1.0 - soft_tau) + param.data * soft_tau)
...
def evaluate_policy(trainer, environment, num_eval_episodes=10, deterministic=True):
    ...
    for _ in range(num_eval_episodes):
        ...
        environment.reset()
        state_dict, reward_dict, cost_dict, _ = environment.initialize_state(render=False)
        ...
        while not done:
            ...
            state_dict, reward_dict, cost_dict, done = environment.step(action_dict, render=False)

        episode_rewards.append(episode_reward)
        episode_costs.append(episode_cost / max(1, episode_steps))

    if not episode_rewards:
        return 0.0, 0.0, 0.0, 0.0

    rewards_array = np.array(episode_rewards, dtype=np.float32)
    costs_array = np.array(episode_costs, dtype=np.float32)
    return float(rewards_array.mean()), float(rewards_array.std(ddof=0)), float(costs_array.mean()), float(costs_array.std(ddof=0))
...
if args.run_name:
    run_id = os.path.join(args.run_name, seed_suffix)
else:
    run_id = os.path.join(param_str, seed_suffix)
model_path = os.path.join(args.save_root, 'model', run_id)
logs_path = os.path.join(args.save_root, 'logs', run_id)
pic_path = os.path.join(args.save_root, 'pic', run_id)
...
if __name__ == '__main__':
    if args.train:
        ...
        for eps in range(args.max_episodes):
            ...
            if eps % args.plot_freq == 0:  # plot and model saving interval
                ...
                np.save(os.path.join(logs_path, 'cost.npy'), cost_episodic)
                np.save(os.path.join(logs_path, 'lambda.npy'), lambda_history)
                ...
                if eps % 5 == 0:
                    ...
                    np.save(os.path.join(logs_path, 'eval_episodes.npy'), eval_episodes)
                    np.save(os.path.join(logs_path, 'eval_mean_rewards.npy'), eval_mean_rewards)
                    np.save(os.path.join(logs_path, 'eval_reward_stds.npy'), eval_reward_stds)
                    np.save(os.path.join(logs_path, 'eval_mean_costs.npy'), eval_mean_costs)
                    np.save(os.path.join(logs_path, 'eval_cost_stds.npy'), eval_cost_stds)
            ...
            print(
            f"Episode: {eps} | Ep Reward: {episode_reward:.2f} | Ep Cost: {episode_cost:.2f} | MA Cost: {sac_trainer.ep_cost_ma:.2f} | Lambda: {F.softplus(sac_trainer.lambda_param).item():.2f}\n"
            f"Duration: {episode_duration:.2f}s | CPU Memory: {psutil.Process().memory_info().rss / 1024 ** 2:.2f} MB | "
            f"GPU Memory: {torch.cuda.memory_allocated() / 1024 ** 2:.2f} MB | "
            f"Replay Buffer: {replay_buffer_usage:.2f}%")
    ...
    if args.eval_sigmas:
        ...
        for eval_sigma in args.eval_sigmas:
            ...
            mean_reward, reward_std, mean_cost, cost_std = evaluate_policy(
                sac_trainer,
                eval_env,
            ...
            print(
                ...
                f"mean_reward={mean_reward:.2f}, std={reward_std:.2f}, mean_cost={mean_cost:.2f}, cost_std={cost_std:.2f}"
            )
    ...
    if args.test:
        ...
        for eps in range(10):
            ...
            while not done:
                ...
                state_dict, reward_dict, cost_dict, done = env.step(action_dict, render=render)
                # env.render()
