...
parser.add_argument('--cost_limit', type=float, default=20.0, help='Cost limit for Lagrange multiplier')
...
experiment_components = [SCRIPT_NAME, sigma_token, f"embed-{args.embedding_mode}", weight_token, f"seed{args.seed}"]
if RUN_NAME:
...
class SAC_Trainer():
    def __init__(self, env, replay_buffer, hidden_dim, action_range, embedding_mode='full'):
        ...
        self.cost_q_net1 = SoftQNetwork(state_dim, action_dim, hidden_dim, embedding_template.clone()).to(device)
        self.cost_q_net2 = SoftQNetwork(state_dim, action_dim, hidden_dim, embedding_template.clone()).to(device)
        self.target_cost_q_net1 = SoftQNetwork(state_dim, action_dim, hidden_dim, embedding_template.clone()).to(device)
        self.target_cost_q_net2 = SoftQNetwork(state_dim, action_dim, hidden_dim, embedding_template.clone()).to(device)
        ...
        self.lambda_param = torch.zeros(1, dtype=torch.float32, requires_grad=True, device=device)
        ...
        for target_param, param in zip(self.target_soft_q_net2.parameters(), self.soft_q_net2.parameters()):
            target_param.data.copy_(param.data)
        for target_param, param in zip(self.target_cost_q_net1.parameters(), self.cost_q_net1.parameters()):
            target_param.data.copy_(param.data)
        for target_param, param in zip(self.target_cost_q_net2.parameters(), self.cost_q_net2.parameters()):
            target_param.data.copy_(param.data)

        self.soft_q_criterion1 = nn.MSELoss()
        self.soft_q_criterion2 = nn.MSELoss()
        self.cost_q_criterion1 = nn.MSELoss()
        self.cost_q_criterion2 = nn.MSELoss()

        soft_q_lr = policy_lr = alpha_lr = args.lr

        self.soft_q_optimizer1 = optim.Adam(self.soft_q_net1.parameters(), lr=soft_q_lr)
        self.soft_q_optimizer2 = optim.Adam(self.soft_q_net2.parameters(), lr=soft_q_lr)
        self.cost_q_optimizer1 = optim.Adam(self.cost_q_net1.parameters(), lr=soft_q_lr)
        self.cost_q_optimizer2 = optim.Adam(self.cost_q_net2.parameters(), lr=soft_q_lr)
        
        self.policy_optimizer = optim.Adam(self.policy_net.parameters(), lr=policy_lr)
        self.alpha_optimizer = optim.Adam([self.log_alpha], lr=alpha_lr)
        self.lambda_optimizer = optim.Adam([self.lambda_param], lr=alpha_lr)
    ...
    def update(self, batch_size,training_steps, reward_scale=10., auto_entropy=True, target_entropy=-2, gamma=0.99, soft_tau=1e-2):
        ...
        cost = torch.FloatTensor(cost).unsqueeze(1).to(device)
        done = torch.FloatTensor(np.float32(done)).unsqueeze(1).to(device)

        predicted_q_value1 = self.soft_q_net1(state, action)
        predicted_q_value2 = self.soft_q_net2(state, action)
        predicted_cost_q_value1 = self.cost_q_net1(state, action)
        predicted_cost_q_value2 = self.cost_q_net2(state, action)
        
        new_action, log_prob, z, mean, log_std = self.policy_net.evaluate(state)
        new_next_action, next_log_prob, _, _, _ = self.policy_net.evaluate(next_state)
        reward = reward_scale * (reward - reward.mean(dim=0)) / (reward.std(dim=0) + 1e-6)  # normalize with batch mean and std; plus a small number to prevent numerical problem
        
        cost_limit_tensor = torch.tensor(self.cost_limit, dtype=torch.float32).to(device)
        ...
        # Training Cost Q Function
        target_cost_q_max = torch.max(self.target_cost_q_net1(next_state, new_next_action),
                                      self.target_cost_q_net2(next_state, new_next_action))
        target_cost_q_value = cost + (1 - done) * gamma * target_cost_q_max
        cost_q_loss1 = self.cost_q_criterion1(predicted_cost_q_value1, target_cost_q_value.detach())
        cost_q_loss2 = self.cost_q_criterion2(predicted_cost_q_value2, target_cost_q_value.detach())
        
        self.cost_q_optimizer1.zero_grad()
        cost_q_loss1.backward()
        if args.use_gradient_clip: torch.nn.utils.clip_grad_norm_(self.cost_q_net1.parameters(), 1.0)
        self.cost_q_optimizer1.step()

        self.cost_q_optimizer2.zero_grad()
        cost_q_loss2.backward()
        if args.use_gradient_clip: torch.nn.utils.clip_grad_norm_(self.cost_q_net2.parameters(), 1.0)
        self.cost_q_optimizer2.step()
        
        # Lambda Update
        lambda_val = F.softplus(self.lambda_param)
        lambda_loss = -(lambda_val * (self.ep_cost_ma - self.cost_limit))
        self.lambda_optimizer.zero_grad()
        lambda_loss.backward()
        # Optionally clip lambda gradients
        self.lambda_optimizer.step()
        lambda_val_detach = F.softplus(self.lambda_param).detach()
        ...
        predicted_new_cost_q_value = torch.max(self.cost_q_net1(state, new_action), self.cost_q_net2(state, new_action))
        
        # Training Policy Function
        if training_steps % args.critic_actor_ratio == 0:
            policy_loss = (self.alpha * log_prob - predicted_new_q_value + lambda_val_detach * predicted_new_cost_q_value).mean()
    ...
    def save_model(self, path):
        ...
        torch.save(self.cost_q_net1.state_dict(), path + '_cost_q1')
        torch.save(self.cost_q_net2.state_dict(), path + '_cost_q2')
    ...
    def load_model(self, path):
        ...
        self.cost_q_net1.load_state_dict(torch.load(path + '_cost_q1', weights_only=True))
        self.cost_q_net2.load_state_dict(torch.load(path + '_cost_q2', weights_only=True))
        self.policy_net.load_state_dict(torch.load(path + '_policy',weights_only=True))

        self.soft_q_net1.eval()
        self.soft_q_net2.eval()
        self.cost_q_net1.eval()
        self.cost_q_net2.eval()
...
def evaluate_policy(sac_trainer, env, num_eval_episodes=5, deterministic=True):
    ...
    for eval_ep in range(num_eval_episodes):
        ...
        eval_costs.append(episode_cost / max(1, episode_steps))

    mean_reward = np.mean(eval_rewards)
    reward_std = np.std(eval_rewards)
    mean_cost = np.mean(eval_costs)
    cost_std = np.std(eval_costs)

    return mean_reward, reward_std, mean_cost, cost_std
...
if __name__ == '__main__':
    if args.train:
        ...
        for eps in range(args.max_episodes):
            if eps != 0:
                env.reset()
            state_dict, reward_dict, cost_dict, _ = env.initialize_state(render=render)
            ...
            while not done:
                for key in state_dict:
                    if len(state_dict[key]) == 1:
                        ...
                        if state_dict[key][0][1] != state_dict[key][1][1]:
                            ...
                            replay_buffer.push(state, action_dict[key], reward, cost, next_state, done)
                            if key == 2 and debug:
                                print('From Algorithm store, Bus id: ', key, ' , station id is: ', state_dict[key][0][1], ' ,current time is: ', env.current_time, ' ,action is: ', action_dict[key], ', reward: ', reward_dict[key], ', cost: ', cost_dict[key],
                                      'value is: ', v_dict[key])
                                print()
                        ...
                        if key == 2 and debug:
                            print('From Algorithm run, Bus id: ', key, ' , station id is: ', state_dict[key][0][1], ' ,current time is: ', env.current_time, ' ,action is: ', action_dict[key], ', reward: ', reward_dict[key], ' ,cost: ', cost_dict[key], ' ,value is: ',
                                  v_dict[key])
                            print()

                state_dict, reward_dict, cost_dict, done = env.step(action_dict, debug=debug, render=render)
                ...
                if done:
                    ...
                    break
            if eps == 0:
                sac_trainer.ep_cost_ma = episode_cost / max(1, episode_steps)
            else:
                sac_trainer.ep_cost_ma = 0.9 * sac_trainer.ep_cost_ma + 0.1 * (episode_cost / max(1, episode_steps))

            # 计算每个 episode 的平均 Q 值
            rewards.append(episode_reward)
            cost_episodic.append(episode_cost / max(1, episode_steps))
            lambda_history.append(F.softplus(sac_trainer.lambda_param).item())
            ...
            if eps % args.plot_freq == 0:  # plot and model saving interval
                ...
                np.save(os.path.join(LOG_DIR, 'cost.npy'), cost_episodic)
                np.save(os.path.join(LOG_DIR, 'lambda.npy'), lambda_history)
                ...
                # 评估当前策略
                mean_reward, reward_std, mean_cost, cost_std = evaluate_policy(sac_trainer, env, num_eval_episodes=15, deterministic=True)
                print(f"评估结果 (Episode {eps}): 平均奖励 = {mean_reward:.2f}, 奖励标准差 = {reward_std:.2f}, 平均Cost = {mean_cost:.2f}, Cost标准差 = {cost_std:.2f}")
                
                #记录评估结果
                eval_episodes.append(eps)
                eval_mean_rewards.append(mean_reward)
                eval_reward_stds.append(reward_std)
                eval_mean_costs.append(mean_cost)
                eval_cost_stds.append(cost_std)
                ...
                np.save(os.path.join(LOG_DIR, 'eval_mean_costs.npy'), eval_mean_costs)
                np.save(os.path.join(LOG_DIR, 'eval_cost_stds.npy'), eval_cost_stds)
            ...
            print(
                f"[SAC-Lagrange | max_alpha={args.maximum_alpha}] Episode: {eps} | Ep Reward: {episode_reward:.2f} | Ep Cost: {episode_cost:.2f} | MA Cost: {sac_trainer.ep_cost_ma:.2f} | Lambda: {F.softplus(sac_trainer.lambda_param).item():.2f}\n"
                f"| CPU: {psutil.Process().memory_info().rss / 1024 ** 2:.2f} MB "
                f"| GPU: {torch.cuda.memory_allocated() / 1024 ** 2:.2f} MB | "
                f"Buffer: {replay_buffer_usage:.2f}%")
          # 在训练结束时保存完整模型（包括critic和actor）
        sac_trainer.save_model(model_path)
        
        # 评估最终策略
        mean_reward, reward_std, mean_cost, cost_std = evaluate_policy(sac_trainer, env, num_eval_episodes=15, deterministic=True)
        print(f"最终评估结果: 平均奖励 = {mean_reward:.2f}, 奖励标准差 = {reward_std:.2f}, 平均Cost = {mean_cost:.2f}, Cost标准差 = {cost_std:.2f}")
        ...
        np.save(os.path.join(final_log_dir, 'eval_mean_costs.npy'), eval_mean_costs)
        np.save(os.path.join(final_log_dir, 'eval_cost_stds.npy'), eval_cost_stds)
    ...
    if args.eval_sigmas:
        ...
        for eval_sigma in args.eval_sigmas:
            ...
            eval_env.reset()
            mean_reward, reward_std, mean_cost, cost_std = evaluate_policy(sac_trainer, eval_env, num_eval_episodes=15, deterministic=True)
    ...
    if args.test:
        ...
        for eps in range(10):
            ...
            while not done:
                ...
                state_dict, reward_dict, cost_dict, done = env.step(action_dict)
                # env.render()
